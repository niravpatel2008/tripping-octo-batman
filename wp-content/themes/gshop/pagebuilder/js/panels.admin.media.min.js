jQuery(function($){
	var originalInsert=wp.media.editor.insert;wp.media.editor.insert=function(h){
		...
		if(h.indexOf('[gallery')!==-1){
			...
			var panel=$('#panels-dialog').panelsCreatePanel('Nemo_Panels_Widgets_Gallery',{'ids':ids.join(',')});
		...
		panel=$('#panels-dialog').panelsCreatePanel('Nemo_Panels_Widgets_Image',{'href':$el.attr('href'),'src':$el.children().eq(0).attr('src')});
	}
	else if($el.prop("tagName")=='IMG'){
		panel=$('#panels-dialog').panelsCreatePanel('Nemo_Panels_Widgets_Image',{'src':$el.attr('src')});
	}
	else if($el.prop('tagName')=='A'&&($el.attr('href').indexOf('.mp4')!==-1||$el.attr('href').indexOf('.avi')!==-1)){
		panel=$('#panels-dialog').panelsCreatePanel('Nemo_Panels_Widgets_Video',{'url':$el.attr('href')});
	}

	if(panel==null)originalInsert(h);else panels.addPanel(panel,null,null,true);return;}
	else{
		var panel=$('#panels-dialog').panelsCreatePanel('WP_Widget_Text',{'text':h});
...
$('body').on({click:function(){
	if(typeof wp==='undefined'||!wp.media||!wp.media.gallery)return;
		event.preventDefault();var $$=$(this);var dialog=$('.panels-admin-dialog:visible');
		var val=dialog.find('*[name$="[ids]"]').val();
		if(val.indexOf('{demo')===0||val.indexOf('{default')===0)val='-';
		if(val==''&&$('#post_ID').val()==null)
			val='-';
		dialog.find('.ui-dialog-content').dialog('close');var frame=wp.media.gallery.edit('[gallery ids="'+val+'"]');
		...
		frame.on('escape',function(){dialog.find('.ui-dialog-content').dialog('open');});return false;
