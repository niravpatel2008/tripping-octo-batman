...
<div id="panels" data-animations="<?php echo rox_panels_setting('animations') ? 'true' : 'false' ?>">
	<div id="panels-container">
	</div>
	
	<div id="add-to-panels">
		<button class="panels-add tooltip"><?php _e('Add Widget', 'so-panels') ?></button>
		<button class="grid-add tooltip"><?php _e('Add Row', 'so-panels') ?></button>
		<?php if(!empty($layouts)) : ?>
			<button class="prebuilt-set tooltip"><?php _e('Prebuilt Layouts', 'so-panels') ?></button>
		<?php endif; ?>
		<div class="clear"></div>
	</div>
	...
	<div id="panels-dialog" data-title="<?php esc_attr_e('Add New Widget','so-panels') ?>" class="panels-admin-dialog">
		<div id="panels-dialog-inner">
			...
			<?php do_action('rox_panels_after_widgets'); ?>
		</div>
    ...
    <script>
	 	function changeDropDown(select){
			jQuery('#grid-add-dialog-input').val(jQuery(select).val());
			}
	</script>
		<p><label><strong><?php _e('Columns', 'so-panels') ?></strong></label></p>
		<p><select  onchange='changeDropDown(this);'>
        ...
        <input type="hidden" id="grid-add-dialog-input" name="column_count" class="small-text" value="2" /></p>
	</div>
	...
	<?php if(!empty($layouts)) : ?>
		<div id="grid-prebuilt-dialog" data-title="<?php esc_attr_e('Insert Prebuilt Page Layout','so-panels') ?>" class="panels-admin-dialog">
			...
			<p>
				<select type="text" id="grid-prebuilt-input" name="prebuilt_layout" class="grid-pagebuild-input-style" placeholder="<?php esc_attr_e('Select Layout', 'so-panels') ?>" >
