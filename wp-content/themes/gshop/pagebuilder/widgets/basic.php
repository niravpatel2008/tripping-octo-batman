...
class Rox_Panels_Widgets_Gallery extends WP_Widget {
	function __construct() {
		parent::__construct(
			'rox-panels-gallery',
			__( 'Gallery (ROX)', 'so-panels' ),
			array(
...
class Rox_Panels_Widgets_PostContent extends WP_Widget {
	function __construct() {
		parent::__construct(
			'rox-panels-post-content',
			__( 'Post Content (ROX)', 'so-panels' ),
			array(
...
class Rox_Panels_Widgets_Image extends WP_Widget {
	function __construct() {
		parent::__construct(
			'rox-panels-image',
			__( 'Image (ROX)', 'so-panels' ),
			array(
 ...
 *
 * Class Rox_Panels_Widgets_PostLoop
 */
class Rox_Panels_Widgets_PostLoop extends WP_Widget{
	function __construct() {
		parent::__construct(
			'rox-panels-postloop',
			__( 'Post Loop (ROX)', 'so-panels' ),
			array(
 ...
 */
class Rox_Panels_Widgets_EmbeddedVideo extends WP_Widget {
	function __construct() {
		parent::__construct(
			'rox-panels-embedded-video',
			__( 'Embedded Video (ROX)', 'so-panels' ),
			array(
	...
	function widget( $args, $instance ) {
		...
		if(!wp_script_is('fitvids'))
			wp_enqueue_script('fitvids', get_template_directory_uri() .'/pagebuilder/widgets/js/jquery.fitvids.min.js', array('jquery'), ROX_PANELS_VERSION);

		if(!wp_script_is('rox-panels-embedded-video'))
			wp_enqueue_script('rox-panels-embedded-video', get_template_directory_uri() .'/pagebuilder/widgets/js/embedded-video.min.js', array('jquery', 'fitvids'), ROX_PANELS_VERSION);

		echo $args['before_widget'];
		?><div class="rox-fitvids"><?php echo $embed->run_shortcode( '[embed]' . $instance['video'] . '[/embed]' ) ?></div><?php
		echo $args['after_widget'];
	}
...
class Rox_Panels_Widgets_Video extends WP_Widget {
	function __construct() {
		parent::__construct(
			'rox-panels-video',
			__( 'Hosted Video (ROX)', 'so-panels' ),
			array(
	...
	function widget( $args, $instance ) {
		...
		wp_enqueue_script( 'rox-panels-video-jplayer', get_template_directory_uri() .'/pagebuilder/video/jplayer/jquery.jplayer.min.min.js', array('jquery'), ROX_PANELS_VERSION, true);
		wp_enqueue_script( 'rox-panels-video', get_template_directory_uri() .'/pagebuilder/video/panels.video.jquery.min.js', array('jquery'), ROX_PANELS_VERSION, true);

		// Enqueue the jPlayer skin
		$skin = sanitize_file_name($instance['skin']);
		wp_enqueue_style('rox-panels-video-jplayer-skin', get_template_directory_uri() .'/pagebuilder/video/jplayer/skins/'.$skin.'/jplayer.'.$skin.'.css', array(), ROX_PANELS_VERSION);

		$file = $instance['url'];
		$poster = !empty($instance['poster']) ? $instance['poster'] :  get_template_directory_uri() .'/pagebuilder/video/poster.jpg';
		...
		<div class="jp-video" id="jp_container_<?php echo $video_widget_id ?>">
			<div class="jp-type-single" id="jp_interface_<?php echo $video_widget_id ?>">
				<div id="jquery_jplayer_<?php echo $video_widget_id ?>" class="jp-jplayer"
				     ...
				     data-swfpath="<?php echo get_template_directory_uri() .'/pagebuilder/video/jplayer/' ?>"
	...
	function display_gui($skin){
		$file = get_template_directory() .'/pagebuilder/video/jplayer/skins/'.$skin.'/gui.php';
		if(file_exists($file)) include get_template_directory() .'/pagebuilder/video/jplayer/skins/'.$skin.'/gui.php';
	}
	...
	function form( $instance ) {
		...
		<p>
			...
			<select id="<?php echo $this->get_field_id('skin') ?>" name="<?php echo $this->get_field_name('skin') ?>">
				<option value="rox" <?php selected($instance['skin'], 'rox') ?>><?php esc_html_e('Nemo', 'so-panels') ?></option>
...
function rox_panels_video_shortcode($atts){
	...
	$instance = shortcode_atts( array(
		'url' => '',
		'src' => '',
		'poster' => get_template_directory_uri() .'/pagebuilder/video/poster.jpg',
		'skin' => 'rox',
		'ratio' => 1.777,
		'autoplay' => 0,
	), $atts );
	...
	ob_start();
	the_widget('Rox_Panels_Widgets_Video', $instance);
	return ob_get_clean();
...
/**
 * A panel that lets you Latest Post.
 */
class Rox_Panels_Widgets_Latest_post extends WP_Widget {
	function __construct() {
		parent::__construct(
			'rox-panels-latest-post',
			__( 'Latest Post (ROX)', 'so-panels' ),
			array(
				'description' => __( 'Display Latest Post With Thumbnails.', 'so-panels' ),
			)
		);
	}
	...
	function widget( $args, $instance ) {
		...
		$html = '';
		if($instance[ 'title' ] != ''){
		$html .= '<div class="title-content"><h3 class="title">'.$instance[ 'title' ] .'</h3></div>';
        }    
		$query_args = array(
		...
		if ( $the_query->have_posts() ) {
			while ( $the_query->have_posts() ) {
				...
				$small_image_url = wp_get_attachment_image_src( get_post_thumbnail_id());
				$large_image_url = wp_get_attachment_image_src( get_post_thumbnail_id());
				$html .= '<div class="latest-posts col-lg-12">';
                if ( has_post_thumbnail() && ! post_password_required() ){
                 $image_url = wp_get_attachment_url( get_post_thumbnail_id($post->ID) );
                    $html .= '<div class="latest-post-thumbnail pull-left">';
	                $html .= '<div class="animation scaleUp">';
					$html .=  get_the_post_thumbnail($post->ID, 'portfolio-thumb' );
					$html .= '</div>
                      </div>';
				}
                 
                  $html .= '<strong><a href="' .get_permalink(). '">';
				  $html .= get_the_title();
				  $html .= '</a></strong>';
                  $content = get_the_content();
                  $word = ($instance[ 'word_count' ] == '')? 10 : $instance[ 'word_count' ];
                  $html .= '<p>' .wp_trim_words( $content, $word ). '</p>';
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'showposts' ) ?>"><?php _e( 'Number Of Posts', 'so-panels' ) ?></label>
			<input type="text" class="widefat" id="<?php echo $this->get_field_id( 'showposts' ) ?>" name="<?php echo $this->get_field_name( 'showposts' ) ?>" value="<?php echo esc_attr($instance['showposts']) ?>" />
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'post_type' ) ?>"><?php _e( 'Post type', 'so-panels' ) ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'cat' ) ?>"><?php _e( 'Cat ID', 'so-panels' ) ?></label>
			<input type="text" class="widefat" id="<?php echo $this->get_field_id( 'cat' ) ?>" name="<?php echo $this->get_field_name( 'cat' ) ?>" value="<?php echo esc_attr($instance['cat']) ?>" />
			<small>comma (,) Separated</small>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'word_count' ) ?>"><?php _e( 'Word Count', 'so-panels' ) ?></label>
			<input type="text" class="widefat" id="<?php echo $this->get_field_id( 'word_count' ) ?>" name="<?php echo $this->get_field_name( 'word_count' ) ?>" value="<?php echo esc_attr($instance['word_count']) ?>" />
 ...
 */
function rox_panels_widgets_init(){
	register_widget('Rox_Panels_Widgets_Gallery');
	register_widget('Rox_Panels_Widgets_PostContent');
	register_widget('Rox_Panels_Widgets_Image');
	//register_widget('Rox_Panels_Widgets_PostLoop');
	register_widget('Rox_Panels_Widgets_EmbeddedVideo');
	register_widget('Rox_Panels_Widgets_Video');
	register_widget('Rox_Panels_Widgets_Latest_post');
}
add_action('widgets_init', 'rox_panels_widgets_init');

/**
 * Enqueue widget compatibility files.
 */
function rox_panels_comatibility_init(){
	
		include get_template_directory() .'/pagebuilder/widgets/compat/tinymce/tinymce.php';
	
}
add_action('admin_init', 'rox_panels_comatibility_init', 5);