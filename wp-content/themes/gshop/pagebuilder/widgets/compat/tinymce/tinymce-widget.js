...
(function($) {
	...
	function black_studio_activate_visual_editor(id) {
		...
		if ( typeof( tinyMCE ) == "object" && typeof( tinyMCE.execCommand ) == "function" ) {
			...
			tinyMCEPreInit.mceInit[id] = tinyMCEPreInit.mceInit['tinymce-widget'];
			tinyMCEPreInit.mceInit[id]["elements"] = id;
			try {
				// Instantiate new TinyMCE editor
				tinymce.init(tinymce.extend( {}, tinyMCEPreInit.mceInit['tinymce-widget'], tinyMCEPreInit.mceInit[id] ));
			} catch(e){
				alert(e);
			}
		}
	...
	function black_studio_open_deferred_activate_visual_editor(id) {
		$('div.widget-inside:has(#' + id + ') input[id^=widget-tinymce][id$=type][value=visual]').each(function() {
			...
			if ($('div.widget:has(#' + id + ') :animated').size() == 0 && typeof(tinyMCE.get(id)) != "object" && $('#' + id).is(':visible')) {
				$('a[id^=widget-tinymce][id$=visual]', $(this).closest('div.widget-inside')).click();
			}
	...
	$(document).ready(function(){
		...
		$(document).on('click', 'div.widget:has(textarea[id^=widget-tinymce]) a.widget-action', function(event){
			//event.preventDefault();
			var $widget = $(this).closest('div.widget');
			var $text_area = $('textarea[id^=widget-tinymce]', $widget);
			...
			$('input[name=savewidget]', $widget).on('click', function(event) {
				...
				$(this).unbind('ajaxSuccess').ajaxSuccess(function(event, xhr, settings) {
					var $text_area = $('textarea[id^=widget-tinymce]', $(this).closest('div.widget-inside'));
		...
		$('div.widget[id*=tinymce] input[name=savewidget]').on('click', function(event) {
			var $widget = $(this).closest('div.widget')
			var $text_area = $('textarea[id^=widget-tinymce]', $widget);
			if (typeof(tinyMCE.get($text_area.attr('id'))) == "object") {
				black_studio_deactivate_visual_editor($text_area.attr('id'));
		...
		$(document).on('click', 'a[id^=widget-tinymce][id$=visual]', function(event) {
			//event.preventDefault();
			var $widget_inside = $(this).closest('div.widget-inside,div.panel-dialog')
			$('input[id^=widget-tinymce][id$=type]', $widget_inside).val('visual');
			$(this).addClass('active');
			$('a[id^=widget-tinymce][id$=html]', $widget_inside).removeClass('active');
			black_studio_activate_visual_editor($('textarea[id^=widget-tinymce]', $widget_inside).attr('id'));
		});
		// Event handler for html switch button
		$(document).on('click', 'a[id^=widget-tinymce][id$=html]', function(event) {
			//event.preventDefault();
			var $widget_inside = $(this).closest('div.widget-inside,div.panel-dialog')
			$('input[id^=widget-tinymce][id$=type]', $widget_inside).val('html');
			$(this).addClass('active');
			$('a[id^=widget-tinymce][id$=visual]', $widget_inside).removeClass('active');
			black_studio_deactivate_visual_editor($('textarea[id^=widget-tinymce]', $widget_inside).attr('id'));
		});
		...
		$(document).on('click', '.editor_media_buttons a', function() {
			...
			wpActiveEditor = $('textarea[id^=widget-tinymce]', $widget_inside).attr('id');	
		});
		// Activate editor when in accessibility mode
		if($('body.widgets_access').size() > 0) {
			var $text_area = $('textarea[id^=widget-tinymce]');
			black_studio_open_deferred_activate_visual_editor($text_area.attr('id'));
		}
	});
