...
include get_template_directory() .'/pagebuilder/widgets/basic.php';
include get_template_directory() .'/pagebuilder/widgets/less/functions.php';

/**
 * Include all the widget files and register their widgets
 */
function origin_widgets_init(){
	foreach(glob(get_template_directory() .'/pagebuilder/widgets/*/*.php') as $file) {
		
		get_template_part('pagebuilder/widgets/'.substr(basename($file), 0, -4));
		...
		$class = 'Rox_Panels_Widget_'.$class;
...
function origin_widgets_enqueue($prefix){
	if($prefix == 'widgets.php') wp_enqueue_script('origin-widgets-admin-script', get_template_directory_uri() .'/pagebuilder/widgets/js/admin.min.js', array('jquery'), ROX_PANELS_VERSION);
}
add_action('admin_enqueue_scripts', 'origin_widgets_enqueue');

/**
 * Class Rox_Panels_Widget
 */
abstract class Rox_Panels_Widget extends WP_Widget{
	public $form_args;
	protected $demo;
	protected $origin_id;
	public $sub_widgets;
	...
	function __construct($name, $widget_options = array(), $control_options = array(), $form = array(), $demo = array()){
		$id_base = str_replace('Rox_Panels_Widget_', '', get_class($this));
	...
	function widget($args, $instance){
		...
		if(!empty($instance['origin_style'])) {
			...
			if( empty( $css_files[$filename] ) || !file_exists($css_files[$filename]['file']) || ( ROX_PANELS_VERSION == 'trunk' && !rox_panels_is_panel() ) ) {
		...
		$widget_classes = apply_filters('rox_widgets_classes', array(
			'origin-widget',
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	// Extra functions specific to a widget.
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function create_css($style, $preset) {
		...
		$less = str_replace(
			'@import "../../../less/mixins";',
			"\n\n".wp_remote_get(plugin_dir_path(__FILE__).'less/mixins.less'),
			$less
		);
	...
	static function get_widget_folders(){
		...
		if(empty($folders)) {
			$folders = array(
				...
				get_template_directory() .'/pagebuilder/widgets/widgets' => get_template_directory_uri() .'/pagebuilder/widgets/widgets',
			);
			$folders = apply_filters('rox_widget_folders', $folders);
		}
	...
	static function get_image_folders(){
		...
		if(empty($folders)) {
			$folders = array(
				...
				get_template_directory() .'/pagebuilder/widgets/img' => get_template_directory_uri() .'/pagebuilder/widgets/img',
			);
			$folders = apply_filters('rox_widget_image_folders', $folders);
		}
	...
	public function get_style_presets($style_id) {
		$filename = get_template_directory() .'/pagebuilder/widgets/'.$this->origin_id.'/presets/'.sanitize_file_name($style_id).'.php';
		if(file_exists($filename)) {
			// This file should register a filter that adds the presets
			
			$presets = get_template_part('pagebuilder/widgets/'.$this->origin_id.'/presets/'.sanitize_file_name($style_id));
		}
