...
class Nemo_Panels_Widgets_Gallery extends WP_Widget {
	...
	function form( $instance ) {
		...
		$types = apply_filters('rox_panels_gallery_types', array());

		$instance = wp_parse_args($instance, array(
			'ids' => '',
			'image_size' => apply_filters('rox_panels_gallery_default_size', ''),
			'type' => apply_filters('rox_panels_gallery_default_type', ''),
		));
...
class Nemo_Panels_Widgets_PostContent extends WP_Widget {
	function __construct() {
		parent::__construct(
			'rox-panels-post-content',
	...
	function widget( $args, $instance ) {
		echo $args['before_widget'];
		$content = apply_filters('rox_panels_widget_post_content', $this->default_content($instance['type']));
		echo $content;
		echo $args['after_widget'];
	}
	...
	function form( $instance ) {
		...
		$types = apply_filters('rox_panels_widget_post_content_types', array(
...
class Nemo_Panels_Widgets_Image extends WP_Widget {
	function __construct() {
		parent::__construct(
			'rox-panels-image',
			__( 'Image', 'so-panels' ),
...
class Nemo_Panels_Widgets_PostLoop extends WP_Widget{
	function __construct() {
		parent::__construct(
			'rox-panels-postloop',
 ...
 */
class Nemo_Panels_Widgets_EmbeddedVideo extends WP_Widget {
	function __construct() {
		parent::__construct(
			'rox-panels-embedded-video',
...
class Nemo_Panels_Widgets_Video extends WP_Widget {
	function __construct() {
		parent::__construct(
			'rox-panels-video',
	...
	function widget( $args, $instance ) {
		...
		wp_enqueue_script( 'rox-panels-video-jplayer', plugin_dir_url(ROX_PANELS_BASE_FILE) . 'video/jplayer/jquery.jplayer.min.min.js', array('jquery'), ROX_PANELS_VERSION, true);
		wp_enqueue_script( 'rox-panels-video', plugin_dir_url(ROX_PANELS_BASE_FILE) . 'video/panels.video.jquery.min.js', array('jquery'), ROX_PANELS_VERSION, true);
		
		// Enqueue the jPlayer skin
		$skin = sanitize_file_name($instance['skin']);
		wp_enqueue_style('rox-panels-video-jplayer-skin', plugin_dir_url(ROX_PANELS_BASE_FILE).'video/jplayer/skins/'.$skin.'/jplayer.'.$skin.'.css', array(), ROX_PANELS_VERSION);

		$file = $instance['url'];
		$poster = !empty($instance['poster']) ? $instance['poster'] :  plugin_dir_url(ROX_PANELS_BASE_FILE).'video/poster.jpg';
		...
		<div class="jp-video" id="jp_container_<?php echo $video_widget_id ?>">
			<div class="jp-type-single" id="jp_interface_<?php echo $video_widget_id ?>">
				<div id="jquery_jplayer_<?php echo $video_widget_id ?>" class="jp-jplayer"
				     ...
				     data-swfpath="<?php echo plugin_dir_url(ROX_PANELS_BASE_FILE).'video/jplayer/' ?>"
	...
	function display_gui($skin){
		$file = plugin_dir_path(ROX_PANELS_BASE_FILE).'video/jplayer/skins/'.$skin.'/gui.php';
		if(file_exists($file)) include plugin_dir_path(ROX_PANELS_BASE_FILE).'video/jplayer/skins/'.$skin.'/gui.php';
	}
	...
	function form( $instance ) {
		...
		<p>
			...
			<select id="<?php echo $this->get_field_id('skin') ?>" name="<?php echo $this->get_field_name('skin') ?>">
				<option value="rox" <?php selected($instance['skin'], 'rox') ?>><?php esc_html_e('SiteOrigin', 'so-panels') ?></option>
 ...
 * @param array $atts
 */
function rox_panels_video_shortcode($atts){
	/**
	 * @var string $url
	 * @var string $poster
	 * @var string $skin
	...
	$instance = shortcode_atts( array(
		'url' => '',
		'src' => '',
		'poster' => plugin_dir_url(ROX_PANELS_BASE_FILE).'video/poster.jpg',
		'skin' => 'rox',
		'ratio' => 1.777,
		'autoplay' => 0,
	), $atts );
	...
	ob_start();
	the_widget('Nemo_Panels_Widgets_Video', $instance);
	return ob_get_clean();
	
}
add_shortcode('self_video', 'rox_panels_video_shortcode');


/**
 * A panel that lets you 3boxes.
 */
class Nemo_Widgets_3boxes extends WP_Widget {
	function __construct() {
		$control_ops = array('width' => 800, 'height' => 800);
		parent::__construct(
			'rox-panels-3boxes',
			__( '3 Boxes (ROX)', 'so-panels' ),
			array(
				'description' => __( 'Display 3 boxes, contain a icon, title, description and detail link field.', 'so-panels' ),
			), $control_ops
		);
	}
	...
	function widget( $args, $instance ) {
		echo $args['before_widget'];
		?>
		<section class="wrapper_padding cbp-so-init cbp-so-animate" id="rox_user_wrapper">
            <div class="container">
                <div class="row-fluid">
                    <div class="rox-user-inner">
                        <!-- Start User Position -->
                        <?php for( $i=1; $i<=3; $i++ ): ?>
                        	<?php $class = ($i==2)? ' rox_border_left rox_border_right': ''; ?>
	                        <div class="span4 rox_left0">
	                            <div class="content_padding clearfix<?php echo $class; ?>">
	                                <img src="<?php echo $instance['box'.$i.'_icon_url']; ?>" alt="Nemo Bootstrap Featured Template">
	                                <h3 class="rox_header"><?php echo $instance['box'.$i.'_title']; ?></h3>
	                                <p><?php echo $instance['box'.$i.'_description']; ?></p> 
	                                 <div class="btnWrap"><a class="roxbtn" href="<?php echo $instance['box'.$i.'_link']; ?>"><?php echo $instance['box'.$i.'_link_text']; ?></a></div>
	                            </div>
	                        </div><!--.span4-->
                        <?php endfor; ?>                        
                        <!-- End User Postion -->
	...
	function form( $instance ) {
	    ...
	    <div class="3boxs"> 
			...
			<div class="box">
				<p>
					<label><?php _e( 'Image URL', 'so-panels' ) ?></label>
					<input class="widefat" type="text" name="<?php echo $this->get_field_name( 'box'.$i.'_icon_url' ) ?>" value="<?php echo esc_attr($instance['box'.$i.'_icon_url']) ?>" /> 
				</p>
				<p>
					<label><?php _e( 'Title', 'so-panels' ) ?></label>
					<input class="widefat" type="text" name="<?php echo $this->get_field_name( 'box'.$i.'_title' ) ?>" value="<?php echo esc_attr($instance['box'.$i.'_title']) ?>" />
				</p>
				<p>
					<label><?php _e( 'Description', 'so-panels' ) ?></label>
					<TEXTAREA  class="widefat" name="<?php echo $this->get_field_name( 'box'.$i.'_description' ) ?>"><?php echo esc_attr($instance['box'.$i.'_description']) ?></TEXTAREA>
				</p>
				<p>
					<label><?php _e( 'Link Text', 'so-panels' ) ?></label>
					<input class="widefat" type="text" name="<?php echo $this->get_field_name( 'box'.$i.'_link_text' ) ?>" value="<?php echo esc_attr($instance['box'.$i.'_link_text']) ?>" />
				</p>
				<p>
					<label><?php _e( 'Link URL', 'so-panels' ) ?></label>
					<input class="widefat" type="text" name="<?php echo $this->get_field_name( 'box'.$i.'_link' ) ?>" value="<?php echo esc_attr($instance['box'.$i.'_link']) ?>" />
				</p>
			</div>	
		<?php endfor; ?>
		</div>
...
/**
 * A panel that lets you 4boxes.
 */
class Nemo_Widgets_4boxes extends WP_Widget {
	function __construct() {
		$control_ops = array('width' => 800, 'height' => 800);
		parent::__construct(
			'rox-panels-4boxes',
			__( '4 Boxes (ROX)', 'so-panels' ),
			array(
				'description' => __( 'Display 4 boxes, contain a icon, title, description.', 'so-panels' ),
			), $control_ops
		);
	}
	...
	function widget( $args, $instance ) {
		echo $args['before_widget'];
		?>
		<section class="wrapper_padding cbp-so-init cbp-so-animate" id="rox_utility_wrapper">
            <div class="container">
                <div class="row-fluid">
					<div class="span12">
	                	<div class="rox_title"><h3 class="rox_header">Our Purpose</h3><div class="rox_title-sep-container"><div class="rox_title-sep"></div></div></div> 
	                    <div class="row-fluid">
	                        <!-- Start User Position -->
	                        <?php 
	                        	$boxclass = array('black', 'green', 'yellow', 'red rox_right0');
	                        	$boxanimate = array('tada', 'pulse', 'fadeOutUp', 'bounceInUp');
	                         ?>
	                        <?php for( $i=1; $i<=4; $i++ ): ?>
		                        <div class="span3 rox_left0" data-animation="<?php echo $boxanimate[$i-1]; ?>">
		                            <div class="content_padding clearfix<?php echo $boxclass[$i-1]; ?>">
		                                <div class="rox_animation"><img src="<?php echo $instance['box'.$i.'_icon_url']; ?>" alt="Nemo Bootstrap"></div>
		                                <h4 class="rox_header"><?php echo $instance['box'.$i.'_title']; ?></h4>
		                                <p><?php echo $instance['box'.$i.'_description']; ?></p> 		                                 
		                            </div>
		                        </div><!--.span4-->
	                        <?php endfor; ?>                        
	...
	function update($new, $old){
		...
		for($i=1; $i<=4; $i++){
			$arr['box'.$i.'_icon_url'] = '';
			$arr['box'.$i.'_title'] = '';
			$arr['box'.$i.'_description'] = '';			
		}
		$new = wp_parse_args($new, $arr);
		return $new;
	}
	...
	function form( $instance ) {
		...
		$instance = wp_parse_args($instance, $arr);
	    ...
	    <div class="3boxs"> 
			...
			<?php for( $i=1; $i<=4; $i++ ): ?>
			<div class="box page-build-widgets-style">
				<p>
					<label><?php _e( 'Image URL', 'so-panels' ) ?></label>
					<input class="widefat" type="text" name="<?php echo $this->get_field_name( 'box'.$i.'_icon_url' ) ?>" value="<?php echo esc_attr($instance['box'.$i.'_icon_url']) ?>" /> 
				</p>
...
function origin_page_builder_widgets_init(){
	register_widget('Nemo_Panels_Widgets_Gallery');
	register_widget('Nemo_Panels_Widgets_PostContent');
	register_widget('Nemo_Panels_Widgets_Image');
	register_widget('Nemo_Panels_Widgets_PostLoop');
	register_widget('Nemo_Panels_Widgets_EmbeddedVideo');
	register_widget('Nemo_Panels_Widgets_Video');

	/*--new--*/
	register_widget('Nemo_Widgets_3boxes');
	register_widget('Nemo_Widgets_4boxes');
}
